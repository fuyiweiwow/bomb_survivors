...
func end_wings(player: Dictionary):
	...
	if game._is_walkable_cell(game.grid[cell.y][cell.x]) and not game.bomb_map.has(cell) and not game.oil_barrels.has(cell):
		return
	for direction in [Vector2i.UP, Vector2i.DOWN, Vector2i.LEFT, Vector2i.RIGHT]:
		...
		if target.x < 0 or target.x >= game.GRID_W or target.y < 0 or target.y >= game.GRID_H:
			continue
		if game._is_walkable_cell(game.grid[target.y][target.x]) and not game.bomb_map.has(target) and not game.oil_barrels.has(target) and not game.wall_mechanics.is_cell_occupied(target):
			player["grid_pos"] = target
			var node = player.get("node")
			if is_instance_valid(node):
				node.position = game._grid_to_world(target)
			return
...
func _use_detonator(player: Dictionary) -> bool:
	...
	for distance in range(1, 7):
		...
		if cell.x < 0 or cell.x >= game.GRID_W or cell.y < 0 or cell.y >= game.GRID_H or game.grid[cell.y][cell.x] == CELL_WALL:
			break
...
func _place_glue(player_index: int):
	...
	if game.glue_areas.has(cell):
		...
		if is_instance_valid(old_node):
			old_node.queue_free()
	var node = game._cylinder(game.TILE_SIZE * 0.38, 0.035, game.mat_glue)
	node.position = game._grid_to_world(cell) + Vector3(0, 0.07, 0)
	game.add_child(node)
...
func _place_oil_barrel(player_index: int) -> bool:
	...
	if cell.x < 0 or cell.x >= game.GRID_W or cell.y < 0 or cell.y >= game.GRID_H:
		return false
	if not game._is_walkable_cell(game.grid[cell.y][cell.x]) or game.bomb_map.has(cell) or game.oil_barrels.has(cell) or game.wall_mechanics.is_cell_occupied(cell):
		player["status"] = "No room for barrel"
		return false
	var root := Node3D.new()
	root.position = game._grid_to_world(cell)
	var body = game._cylinder(0.48, 0.92, game.mat_oil)
	body.position = Vector3(0, 0.46, 0)
	root.add_child(body)
	var band = game._cylinder(0.50, 0.10, game.mat_bomb_power)
...
func _cast_tianlao(player_index: int):
	...
	for raw_cell in cells:
		var marker = game._box(Vector3(game.TILE_SIZE * 0.72, 0.06, game.TILE_SIZE * 0.72), game.mat_bomb_power)
		marker.position = game._grid_to_world(raw_cell as Vector2i) + Vector3(0, 0.10, 0)
