...
const DIRECT_USE_ITEM_MANAGER := preload("res://scripts/item/direct_use_item_manager.gd")
const ROCK_ATTACK_CONTROLLER := preload("res://scripts/item/rock_attack_controller.gd")
...
const CHARACTER_PRESENTATION := preload("res://scripts/character/character_presentation.gd")
...
static func install(game: Node) -> void:
	...
	game.direct_use_item_manager = _install(game, DIRECT_USE_ITEM_MANAGER)
	game.rock_attack_controller = _install(game, ROCK_ATTACK_CONTROLLER)
	game.grid_manager = _install(game, GRID_MANAGER, false)
	game.grid_manager.setup(game, game.art.terrain_materials())
	game.character_presentation = _install(game, CHARACTER_PRESENTATION)
