...
func _run() -> void:
	...
	if not _check(menu.guide_overlay.operation_text.text.contains("player has 3 HP") and menu.guide_overlay.operation_text.text.contains("regular AI has 1 HP") and menu.guide_overlay.operation_text.text.contains("hits remove 1 HP"), "Operation guide is missing the player, AI, or damage health rules"):
		return
	if not _check(menu.guide_overlay.operation_text.text.contains("X") and menu.guide_overlay.operation_text.text.contains("Discard the equipped direct-use item"), "Operation guide is missing the direct-use discard control"):
		return
	var expected_item_ids := ["speed", "bomb", "range", "health", "shield", "rock", "detonator", "glue", "shield_potion", "invincible_star", "dummy", "oil_barrel", "wings", "football_shoes", "prison", "duel"]
	...
	if not _check(item_label_texts.has("Prison") and not item_label_texts.has("Tianlao"), "Item guide did not use the English Prison display name"):
		return
	if not _check(item_label_texts.has("Health") and item_label_texts.has("Direct-use Equipment") and item_label_texts.has("Active Backpack Items"), "Item guide did not separate active items, direct-use equipment, and Health"):
		return
